...
const MovieDetail = () => {
  const [details,setDetails] = useState<ITrendings[]>([])
  const param = useParams()
  useEffect(()=>{
    (async()=>{
      try {
        const {data}= await axios.get(`http://127.0.0.1:8000/api/phim-dang-chieu/${param?.id as number|string}`)
        setDetails(data)
      } catch (error) {
        console.log(error);
  ...
  {Array.isArray(details) && details.map((detail:ITrendings)=>(
      ...
      <div className="row">
          <div className="col-lg-3">
            <div className="anime__details__pic set-bg" data-setbg="img/trending/2.jpg">
              <div className="comment"><i className="fa fa-comments" /> {detail.duration}</div>
              <div className="view"><i className="fa fa-eye" /> {detail.nation}</div>
            </div>
          </div>
          <div className="col-lg-9">
            <div className="anime__details__text">
              <div className="anime__details__title">
                <h3>{detail.name_movie}</h3>
